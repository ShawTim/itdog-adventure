...
const rise = () => {
  state.allBodies.forEach((body) => {
    Matter.Body.setVelocity(body, Matter.Vector.create(0, -5));
  });
}
...
const createNewBlock = () => {
  ...
  node.style = `position: absolute; left: 0px`;
...
document.addEventListener("DOMContentLoaded", (e) => {
  ...
  container.addEventListener("keyup", (e) => rise());
  container.addEventListener("blur", function(e) { setTimeout(() => this.focus(), 0) });
  container.focus();
  initRender();
});
